...
// https://infocenter.nordicsemi.com/topic/com.nordic.infocenter.s332.api.v2.0.0/group__ant__interface.html
...
static const uint8_t antkeys[KEY_TOTAL][KEY_SIZE] = {
...
typedef struct {
	int (*cbPtr) (const int channel, const int messageId, const uint8_t *payLoad, const size_t dataLength, void *userPtr);
	void *uPtr;			// user variable sent with each event
}TEVENTFUNC;

typedef struct {
	void (*cbPtr) (TDCONFIG *cfg, const uint8_t *payLoad, const size_t dataLength, void *userPtr);
...
typedef struct {
	...
	//uint16_t vid;
	//uint16_t pid;
	int iDevice;					// index to the antplus we're interested in, if more than one are found
